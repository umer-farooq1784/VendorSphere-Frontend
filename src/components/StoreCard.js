...
const StoreCard = ({ id, storeDetails }) => {
  const navigate = useNavigate()

  const openDescriptionPage = (e) => {
    navigate(`/storeDescription/${storeDetails?.id}`)
  }

  return (
    <div className="group mr-2 max-w-56 md:mr-4 my-3 md:my-10 flex w-full  flex-col rounded-lg border border-gray-100 bg-white shadow-md">
      <div
      ...
      >
        <img
          className={`peer absolute top-0 right-0 h-full w-full object-cover transition-opacity delay-1000 duration-100 ${
            storeDetails?.ProductImages[1] ? 'group-hover:opacity-0 peer-hover:opacity-0' : ''
          }`}
          src={`${storeDetails?.ProductImages[0]?.image_url}`}
          alt="product image"
        />

        {storeDetails?.ProductImages[1] && (
          <img
            ...
            src={`${storeDetails?.ProductImages[1]?.image_url}`}
            alt="product image"
          />
        )}
        <svg
      ...
      <div className="mt-4 px-5 pb-5">
        <div className="">
          <p className="text-lg font-bold md:text-xl tracking-tight text-slate-900">
            {storeDetails?.name} - {storeDetails?.ProductCategory?.name}
          </p>
          <div className="flex justify-between">
            <p className="text-sm tracking-tight text-slate-900">
              {storeDetails?.ProductCategory?.description}
            </p>
            <div className="flex">
